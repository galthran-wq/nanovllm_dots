"""nanovllm_dots.engine.llm_engine
...
- Request state: :class:`~nanovllm_dots.engine.sequence.Sequence`
  (prompt, generated tokens, KV bookkeeping).
- Scheduling + KV allocation: :class:`~nanovllm_dots.engine.scheduler.Scheduler`
  (batching policy) and :class:`~nanovllm_dots.engine.block_manager.BlockManager`
  (KV block pool + prefix caching).
- GPU execution: :class:`~nanovllm_dots.engine.model_runner.BaseModelRunner`
...
2) The model-specific engine converts each selected :class:`Sequence` into a
   :class:`~nanovllm_dots.engine.model_runner.RunnerTask` via
   :meth:`preprocess_seq`.
...
See the model packages under ``nanovllm_dots/models/*`` for concrete
implementations.

Concrete example: VoxCPM
------------------------
``nanovllm_dots/models/voxcpm/engine.py`` implements an audio generation loop
...
from nanovllm_dots.engine.lora_manager import LoRAManager, LoRAModelPayload
from nanovllm_dots.engine.sequence import Sequence
from nanovllm_dots.engine.scheduler import Scheduler
from nanovllm_dots.engine.model_runner import RunnerTask, BaseModelRunner, select_lora_payload_for_rank
import socket
import torch
...
class LLMEngineBase:
    ...
    def preprocess_seq(self, seq: Sequence, is_prefill: bool) -> RunnerTask:
        """Convert a scheduled :class:`~nanovllm_dots.engine.sequence.Sequence` into a runner input.

        The engine loop is model-agnostic, so each model family must implement
        how to build a :class:`~nanovllm_dots.engine.model_runner.RunnerTask`
        ...
        Concrete example: VoxCPM
        ------------------------
        See ``nanovllm_dots/models/voxcpm/engine.py``:
    ...
    def postprocess_seq(self, seq: Sequence, outputs: dict, is_prefill: bool):
        """Merge runner outputs back into the :class:`~nanovllm_dots.engine.sequence.Sequence`.
        ...
        - Do not allocate/free KV blocks here. KV lifecycle is owned by
          :class:`~nanovllm_dots.engine.scheduler.Scheduler` /
          :class:`~nanovllm_dots.engine.block_manager.BlockManager`.

        Concrete example: VoxCPM
        ------------------------
        In ``nanovllm_dots/models/voxcpm/engine.py``:
