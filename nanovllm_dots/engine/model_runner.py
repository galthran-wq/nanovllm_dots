"""nanovllm_dots.engine.model_runner

This module defines the GPU execution abstraction used by the engine.

The high-level runtime separates concerns:
- :mod:`nanovllm_dots.engine.scheduler` decides *what to run* (which sequences)
  and manages KV-cache block allocation.
- :mod:`nanovllm_dots.engine.llm_engine` orchestrates the step loop and
...
tensors stored inside causal :class:`~nanovllm_dots.layers.attention.Attention`
...
- Prepare attention metadata ("context") for flash-attn kernels via
  :func:`nanovllm_dots.utils.context.set_context`.
...
------------------------
``nanovllm_dots/models/voxcpm/runner.py`` shows a typical implementation:
...
from nanovllm_dots.engine.lora_manager import (
    LoRAModelPayload,
    LoRARuntime,
...
from nanovllm_dots.layers.lora import iter_lora_modules
from nanovllm_dots.lora import is_available as is_lora_available
from nanovllm_dots.utils.context import (
    DIT_LORA_DOMAIN,
    LM_LORA_DOMAIN,
    PROJ_LORA_DOMAIN,
    LoRAContext,
