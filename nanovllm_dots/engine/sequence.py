...
2) The :class:`~nanovllm_dots.engine.scheduler.Scheduler` transitions sequences
...
- ``num_cached_tokens``: Count of prompt tokens whose KV state is reused from
  the prefix cache (see :mod:`nanovllm_dots.engine.block_manager`).
...
------------------------
The VoxCPM implementation under ``nanovllm_dots/models/voxcpm`` uses
