"""Native DiT (flow-matching / MeanFlow velocity field predictor), ported from the
reference ``dots_tts/modules/backbone/dit.py``.

Structurally identical to the reference (same submodule + attribute names) so the
engine's FM accelerators (flash_cached_fm / cached_fm / batched_fm / graphed_meanflow
/ cudagraph_dit) bind to ``dit.blocks[l].attn.{q_proj,k_proj,v_proj,q_norm,k_norm,
rotary,o_proj}``, ``dit.time_embedder``, ``dit.input_layer``,
``dit.output_layer.{norm,adaLN_modulation,linear}`` unchanged, and reference weights
load with a direct state-dict copy. Validated native==reference at cos~1.0.
"""
import math
...
class DiTBlock(nn.Module):
    def __init__(self, attention: nn.Module, ffn: nn.Module, hidden_size: int = 1024,
                 modulation: bool = False, eps: float = 1e-5, **_kwargs):
        super().__init__()
    ...
    def forward(self, x, condition=None, mask=None, **kwargs):
        ...
        if condition is not None:
            x = x + gate_attn * self.attn(
                modulate(self.norm1(x), shift_attn, scale_attn, **kwargs), mask=mask, **kwargs)
        else:
...
class DiT(nn.Module):
    def __init__(self, in_dim, out_dim, transformer_config, *, mode: str = "flow_matching"):
        super().__init__()
    ...
    def forward(self, x, timesteps, duration: torch.Tensor | None = None, mask=None,
                attn_mask=None, g_cond: torch.Tensor | None = None, **kwargs):
