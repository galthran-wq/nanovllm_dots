"""Backbone layers, ported verbatim from the reference
``dots_tts/modules/backbone/layers.py`` (which itself depends only on torch +
einops -- no dots_tts imports). Shared by the native DiT and patch_encoder.

Kept structurally identical (same class/attribute names, unfused q/k/v) so the
engine's accelerators bind to the same leaves and the reference weights load
directly. Validated native==reference at cos~1.0.
"""
import torch
...
class Dropout(nn.Module):
    def __init__(self, p: float = 0.5, inplace: bool = False, force_drop: bool = False,
                 **_kwargs):
        super().__init__()
        if p < 0.0 or p > 1.0:
...
class Conv1d(nn.Conv1d):
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int = 1,
                 ...
                 padding_mode: str = "zeros", bias: bool = True, padding=None,
                 causal: bool = False, **_kwargs):
        self.causal = causal
        if padding is None:
            if causal:
                padding = 0
...
class ConvTranspose1d(nn.ConvTranspose1d):
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int,
                 stride: int = 1, output_padding: int = 0, groups: int = 1,
                 bias: bool = True, dilation: int = 1, padding=None,
                 padding_mode: str = "zeros", causal: bool = False, **_kwargs):
        if padding is None:
...
class Mlp(nn.Module):
    def __init__(self, hidden_size, ffn_hidden_size=4096, act_layer=nn.GELU,
                 dropout=0.0, **_kwargs):
        super().__init__()
...
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, t):
        ...
        if inv_freq.device != t.device:
            raise RuntimeError("RotaryEmbedding buffer device mismatch: "
...
class MultiHeadAttention(nn.Module):
    """Multi-head attention (unfused q/k/v -- the engine accelerators bind to
    q_proj/k_proj/v_proj/q_norm/k_norm/rotary/o_proj individually)."""

    def __init__(self, hidden_size: int, num_heads: int = 8, qkv_bias: bool = False,
                 qk_norm: bool = False, attn_drop: float = 0.0, dropout: float = 0.0,
                 norm_layer: str = "LayerNorm", rotary_bias: bool = False,
                 rotary_theta: float | None = 50000, **_kwargs):
        super().__init__()
    ...
    def decode_step(self, x, *, cache, positions: torch.Tensor):
        ...
        key_positions = torch.arange(cache_capacity, device=x.device, dtype=torch.long).unsqueeze(0)
        query_positions = positions.unsqueeze(1)
        causal_mask = key_positions <= query_positions
        valid_mask = key_positions <= positions[-1]
        attn_bias = torch.zeros(q.size(0), self.num_heads, block_len, cache_capacity,
                                dtype=q.dtype, device=q.device)
        attn_bias.masked_fill_(
            (causal_mask & valid_mask).unsqueeze(0).unsqueeze(0).logical_not(), float("-inf"))

        out = F.scaled_dot_product_attention(
            q, cached_k, cached_v, attn_mask=attn_bias,
            dropout_p=self.attn_drop.p if self.training else 0.0)
        out = rearrange(out, "b h n d -> b n (h d)")
