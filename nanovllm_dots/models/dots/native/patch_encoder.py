"""Native VAE semantic encoder (the patch_encoder), ported from the reference
``dots_tts/modules/backbone/semantic_encoder.py``.

Structurally identical to the reference (same submodule + attribute names) so the
engine + FlashPatchEncoder bind to ``pe.{in_proj,ds_proj,_downsample,_downsample_step,
_project_embeddings,out_ds_rate}`` and ``pe.encoder.layers[l].{attn,attn_norm,ffn_norm,
ffn}`` unchanged, and reference weights load directly. Validated native==reference.
"""
...
from .layers import Conv1d, Mlp, MultiHeadAttention
...
class TransformerEncoderLayer(nn.Module):
    def __init__(self, hidden_size, num_heads=16, ffn_hidden_size=4096, attn_dropout=0.0,
                 ffn_dropout=0.0, norm_layer="LayerNorm", **kwargs):
        super().__init__()
        self.attn = MultiHeadAttention(hidden_size, num_heads, attn_drop=attn_dropout,
    ...
    def forward(self, x, x_lens=None, causal=True):
        ...
        padding_mask = self._build_padding_mask(x_lens, T, device) if x_lens is not None else None
        fused_mask = self._fuse_attn_mask(causal_mask, padding_mask)
        h = self.attn_norm(x)
        h = self.attn(q=h, mask=fused_mask)
        x = x + h
        h = self.ffn_norm(x)
        h = self.ffn(h)
        return x + h

    def decode_step(self, x, *, cache: tuple[torch.Tensor, torch.Tensor], positions: torch.Tensor):
        if x.size(1) <= 0:
...
class SuperviseEncoder(nn.Module):
    ...
    def init_decode_state(self, *, batch_size: int, max_seq_len: int,
                          device: torch.device, dtype: torch.dtype):
        layer_caches = []
        for layer in self.layers:
            cache_shape = (batch_size, layer.attn.num_heads, max_seq_len, layer.attn.head_dim)
            layer_caches.append((torch.zeros(cache_shape, dtype=dtype, device=device),
                                 torch.zeros(cache_shape, dtype=dtype, device=device)))
        return tuple(layer_caches)

    def reset_decode_state(self, layer_caches) -> None:
...
class VAESemanticEncoder(nn.Module):
    ...
    def init_decode_state(self, *, max_audio_patch_count: int, batch_size: int,
                          device: torch.device, dtype: torch.dtype) -> SemanticEncoderDecodeState:
        return SemanticEncoderDecodeState(
            conv_tail=torch.zeros((batch_size, self.ds_proj.in_channels, self.ds_proj.left_padding),
                                  dtype=dtype, device=device),
            layer_caches=self.encoder.init_decode_state(
                batch_size=batch_size, max_seq_len=max_audio_patch_count * self.out_ds_rate,
    ...
    def prefill(self, x, state: SemanticEncoderDecodeState):
        if x.ndim != 3:
        ...
        if x.size(1) == 0:
            return x.new_zeros((x.size(0), 0, self.out_proj.out_features)), state
        ...
        if step_inputs.size(1) != expected_token_count:
            raise RuntimeError("Patch encoder prefill produced an unexpected token count: "
        ...
        if next_seq_len > cache_capacity:
            raise ValueError("Patch encoder prefill exceeds decode-state capacity: "
        ...
        encoded = self.encoder.decode_step(step_inputs, layer_caches=state.layer_caches, positions=positions)
    ...
    def decode_patch(self, latent_patch, conv_tail: torch.Tensor, layer_caches, positions: torch.Tensor):
        ...
        step_inputs, conv_tail = self._downsample_step(latent_patch, conv_tail=conv_tail)
        if step_inputs.size(1) != self.out_ds_rate:
            raise RuntimeError(
                f"Downsample step produced {step_inputs.size(1)} tokens, expected {self.out_ds_rate}.")
        encoded = self.encoder.decode_step(step_inputs, layer_caches=layer_caches, positions=positions)
        embedding = self._project_embeddings(encoded)
    ...
    def _downsample_step(self, latent_patch, *, conv_tail):
        ...
        projected = F.conv1d(conv_input, self.ds_proj.weight, self.ds_proj.bias,
                             stride=self.ds_proj.stride[0], padding=0,
                             dilation=self.ds_proj.dilation[0], groups=self.ds_proj.groups).transpose(1, 2)
